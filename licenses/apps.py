...
def config_check(app_configs, **kwargs):
    ...
    if not settings.DEBUG and settings.SECRET_KEY == settings.DEV_SECRET_KEY:
        return [Error("config_invalid: LICENSE_SESSION_SECRET must be set when "
                      "LICENSE_DEBUG=0 (no hardcoded production default).", id="licenses.E001")]
    try:
        from django.db import connections
        connections["default"].ensure_connection()
    except Exception as exc:
        return [Error(f"store_unavailable: cannot open the License Store at startup: {exc}",
                      id="licenses.E002")]
    return []
