...
def login_page(request):
    if request.method == "POST":
        try:
            user = services.authenticate_account(request, request.POST.get("username"),
...
def home(request):
    return render(request, "licenses/home.html",
                  {"entitlements": request.user.entitlements.order_by("pk")})
...
def entitlement_page(request, entitlement_id):
    ...
    if request.method == "POST":
        services.bind(entitlement, request.POST.get("device_fingerprint", ""),
                      request.POST.get("display_name") or None)
        return redirect("ui_entitlement", entitlement_id=entitlement.pk)
    bound = entitlement.devices.filter(status="bound").count()
    return render(request, "licenses/entitlement.html",
                  {"entitlement": entitlement, "devices": entitlement.devices.order_by("pk"),
                   "seats_used": bound, "seats_free": entitlement.max_devices - bound})
